'''
	finish push/pop classes
	fill missing code
'''


import fileinput, sys, os
	
def error(msg):
	raw_input('ERROR: ' + msg)
	sys.exit()
	
commands_arithmetic = {
	'add': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'A=A-1'		,
		'M=D+M'		]
	,
	'sub': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'A=A-1'		,
		'M=M-D'		]
	,
	'neg': [
		'@SP'		,
		'A=M-1'		,
		'M=-M'		]
	,
	'eq': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'@SP'		,
		'A=M-1'		,
		'D=M-D'		,
		'@{0}'		,
		'D;JEQ'		,
		'@0'		,
		'D=!A'		,
		'@SP'		,
		'A=M-1'		,
		'M=!D'		]
	,
	'gt': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'@SP'		,
		'A=M-1'		,
		'D=D-M'		,
		'@32767'	,
		'A=!A'		,
		'D=D&A'		,
		'@{0}'		,
		'D;JEQ'		,
		'@0'		,
		'D=!A'		,
		'@SP'		,
		'A=M-1'		,
		'M=D'		]
	,
	'lt': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'@SP'		,
		'A=M-1'		,
		'D=M-D'		,
		'@32767'	,
		'A=!A'		,
		'D=D&A'		,
		'@{0}'		,
		'D;JEQ'		,
		'@0'		,
		'D=!A'		,
		'@SP'		,
		'A=M-1'		,
		'M=D'		]
	,
	'and': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'@SP'		,
		'A=M-1'		,
		'M=D&M'		]
	,
	'or': [
		'@SP'		,
		'AM=M-1'	,
		'D=M'		,
		'@SP'		,
		'A=M-1'		,
		'M=D|M'		]
	,
	'not': [
		'@SP'		,
		'A=M-1'		,
		'M=!M'		]
	,
}
		
segment = {
	'local': '@LCL'		,
	'argument': '@ARG'	,
	'this': '@THIS'		,
	'that':	'@THAT'		,
}
	
class C_Base():
	def __init__(self, filename, id, arg1=None, arg2=None):
		self.filename = filename
		self.id = id
		self.arg1 = arg1
		self.arg2 = arg2
		self.info = '{} {} {}'.format(id,arg1,arg2)
		
class C_Arithmetic(C_Base):
	def getassembly(self, currentline):
		r = ['//'+self.info]
		
		for code in commands_arithmetic[self.id]:
			currentline += 1
			r.append(code.format(currentline+3))
			
		return r
	
	@property
	def linecount(self):
		return len(commands_arithmetic[self.id])
	
class C_Push(C_Base):
	def getassembly(self, currentline):
		r = ['//'+self.info]
		
		if self.arg1 == 'static':
			r.extend([
				'@'+self.filename+'.'+self.arg2,
				'D=M'
			])
		else:
			r.extend([
				'@'+self.arg2	,
				'D=A'			]
			)
		
			if self.arg1 == 'temp':
				r.extend([
					'@5'			,
					'A=A+D'			,
					'D=M'			]
				)
			elif self.arg1 == 'pointer':
				r.extend([
					'@3'			,
					'A=A+D'			,
					'D=M'			]
				)
			elif self.arg1 != 'constant':
				r.extend([
					segment[self.arg1]	,
					'A=M+D'				,
					'D=M'				]
				)
		
		r.extend([
			'@SP'		,
			'M=M+1'		,
			'A=M-1'		,
			'M=D'		]
		)
			
		return r
		
	@property
	def linecount(self):
		if self.arg1 == 'constant':
			return 6
		elif self.arg1 == 'static':
			return 6
		else:
			return 9

class C_Pop(C_Base):
	def getassembly(self, currentline):
		r = ['//'+self.info]
		
		if self.arg1 == 'static':
			r.extend([
				'@SP',
				'AM=M-1',
				'D=M',
				'@'+self.filename+'.'+self.arg2,
				'M=D',
			])
			
		else:
			r.extend([
				'@'+self.arg2		,
				'D=A'				]
			)
			
			if self.arg1 == 'temp':
				r.extend([
					'@5'	,
					'D=D+A'	]
				)
			elif self.arg1 == 'pointer':
				r.extend([
					'@3'	,
					'D=D+A'	]
				)
			else:
				r.extend([
					segment[self.arg1]	,
					'D=M+D'				]
				)
			
			r.extend([
				'@R13'		,
				'M=D'		,
				'@SP'		,
				'AM=M-1'	,
				'D=M'		,
				'@R13'		,
				'A=M'		,
				'M=D'		]
			)
			
		return r
		
	@property
	def linecount(self):
		if self.arg1 == 'static':
			return 5
		else:
			return 14
		

class C_Label(C_Base):
	pass
	
class C_Goto(C_Base):
	pass

class C_If(C_Base):
	pass
	
class C_Function(C_Base):
	pass
	
class C_Return(C_Base):
	pass
	
class C_Call(C_Base):
	pass
		
def tokenize(line):
	r = line.strip('\n').partition('//')[0].split(None,2)
	if r:
		if len(r) > 2:
			return True, r[0], r[1], r[2]
		elif len(r) > 1:
			return True, r[0], r[1], None
		elif len(r) > 0:
			return True, r[0], None, None
	return False, None, None, None
		
def parse(files,filename):
	r = []
	
	for line in fileinput.input(files):
		ok, id, arg1, arg2 = tokenize(line)
		if ok:
			if id in commands_arithmetic:
				r.append(C_Arithmetic(filename,id,arg1,arg2))
			elif id == 'push':
				r.append(C_Push(filename,id,arg1,arg2))
			elif id == 'pop':
				r.append(C_Pop(filename,id,arg1,arg2))
			else:
				error('invalid id: ' + id)
			
	return r
	
def code(parsed):
	coded, i = [], 0
	
	for command in parsed:
		coded.extend(command.getassembly(i))
		i = i + command.linecount

	return coded
			

if __name__ == "__main__":
	#get file
	filename = raw_input('File/Folder name:')
	path = sys.path[0] + '\\' + filename
	if os.path.isdir(path):
		files = os.listdir(path)
	elif os.path.isfile(path + '.vm'):
		files = (path + '.vm')
	else:
		error('File not found')
			
	parsed = parse(files,filename)
	coded = code(parsed)
	
	#combine strings
	combined = ''
	for i in range(len(coded)):
		print(i,coded[i])
		combined = combined + coded[i] + '\n'
	
	#open new file
	file = open(path + '.asm', 'w')
	file.write(combined)